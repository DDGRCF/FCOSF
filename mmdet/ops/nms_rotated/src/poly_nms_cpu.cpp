...
#include "poly_iou_rotated_utils.h"


template <typename scalar_t>
at::Tensor nms_poly_cpu_kernel(
  ...
  for (int64_t _i = 0; _i < ndets; _i++) {
    ...
    for (int64_t _j = _i + 1; _j < ndets; _j++) {
      ...
      auto ovr = single_poly_iou_rotated<scalar_t>(
...
at::Tensor nms_poly_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES(dets.scalar_type(), "nms_poly", [&] {
    result = nms_poly_cpu_kernel<scalar_t>(dets, scores, iou_threshold);
  });
  return result;
}
